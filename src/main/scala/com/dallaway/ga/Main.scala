...
object GA {
  ...
  def evaluate(population: Population, f: Individual => Int): PopulationFitness =
    population.individuals.map(f)

  // roulette-wheel sampling
  def select(pop: Population, fitness: PopulationFitness): (Individual, Individual) = {
    ...
    def select1(): Individual = {
      ...
      wheel.dropWhile{ case (fitness, _) => fitness < number}.map { case (_, individual) => individual}.head
    }
  ...
  def evolve(params: MetaParams, pop: Population, fitness: PopulationFitness): Population = {
    ...
    Population(Vector.fill(params.popSize / 2)(create2children).collect { case (c1, c2) => Vector(c1,c2) }.flatten)
  }
